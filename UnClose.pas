...
procedure TForm1.Timer1Timer(Sender: TObject);
 ...
 begin
  ...
  if handle <> 0 then  begin
   @plugExec := GetProcAddress(handle,'PlExec');
   @plugType := GetProcAddress(handle,'PlType');
   if PlugType = 'TEST' then if not PlugExec(nil) then SavesLog('Спектрометр: Ошибка доступа к файлу!');
   // else if PlugType = 'NIL' then PlugExec(nil);
  end; //if
  FreeLibrary(handle);
 end; //procedure
...
begin
 //if MessageBox(Handle,PChar('вы уверены?'),PChar('Сообщение'),MB_ICONWARNING+MB_YESNO)=ID_YES then ;
end;
